#property copyright "TTM FVG HTF Alert v7"
#property link      ""
#property version   "1.00"
#property indicator_chart_window
#property indicator_plots 0

#define PREFIX "TTM_FVG_HTF_ALERT_V7_"
#define DIR_BULL 1
#define DIR_BEAR -1
#define GRADE_C 1
#define GRADE_B 2
...
input int HtfNearDistancePoints = 300;
input int HtfMaxAgeBars = 120;
...
string HtfFvgSummary(const ENUM_TIMEFRAMES timeframe, const string timeframeName)
{
   ...
   int maxShift = MathMin(MathMin(HtfMaxBarsToScan, HtfMaxAgeBars), bars - 3);
   ...
   for(int shift = 1; shift <= maxShift; shift++)
   {
      ...
      double distance = 0.0;
      bool inside = currentPrice <= top && currentPrice >= bottom;
      if(currentPrice > top)
         distance = currentPrice - top;
      else if(currentPrice < bottom)
         distance = bottom - currentPrice;

      if(!inside && distance > HtfNearDistancePoints * PointValue())
         continue;
   ...
   if(!found)
      return timeframeName + ": none nearby";

   if(nearestInside)
      return timeframeName + ": INSIDE " + DirectionText(nearestDirection) + " " + GradeText(nearestGrade) + " | " + IntegerToString(nearestAge) + " bars";

   return timeframeName + ": NEAR " + DirectionText(nearestDirection) + " " + GradeText(nearestGrade) + " | " + DoubleToString(nearestDistance / PointValue(), 0) + " pts";
}
...
{
   DrawLabel(PREFIX + "DIAG_TITLE", 12, 20, "TTM FVG HTF Alert v7", TextColor);
   ...
   if(ShowHtfFvgPanel)
   {
      DrawLabel(PREFIX + "DIAG_HTF_TITLE", 12, 164, "HTF FVG Proximity:", TextColor);
...
int OnInit()
{
   IndicatorSetString(INDICATOR_SHORTNAME, "TTM FVG HTF Alert v7");
