#property copyright "TTM FVG Freshness v3"
#property link      ""
#property version   "1.00"
#property indicator_chart_window
#property indicator_plots 0

#define PREFIX "TTM_FVG_FRESHNESS_V3_"
#define DIR_BULL 1
#define DIR_BEAR -1
#define GRADE_C 1
#define GRADE_B 2
...
{
   DrawLabel(PREFIX + "DIAG_TITLE", 12, 20, "TTM FVG Freshness v3", TextColor);
   ...
   DrawLabel(PREFIX + "DIAG_OBJECTS", 12, 128, "Objects created: " + IntegerToString(g_objectsCreated), TextColor);
   DrawLabel(PREFIX + "DIAG_FAILURES", 12, 146, "Create failures: " + IntegerToString(g_objectCreateFailures), TextColor);
   DrawLabel(PREFIX + "DIAG_ERROR", 12, 164, "Last object error: " + IntegerToString(g_lastObjectError), TextColor);
}
...
void ScanFVGs(const int rates_total, const datetime &time[], const double &high[], const double &low[], const double &close[])
{
   ...
   for(int shift = maxShift; shift >= 1; shift--)
   {
      ...
      int grade = ApplyFreshnessDecay(originalGrade, ageBars);
...
void DrawFVG(const BasicFVG &fvg)
{
   ...
   if(ShowStatusText)
      DrawText(id + "_TEXT", fvg.labelTime, labelPrice, directionText + " FVG" + gradeText + " - " + statusText + ageText, TextColor);
}
...
int OnInit()
{
   IndicatorSetString(INDICATOR_SHORTNAME, "TTM FVG Freshness v3");
