#property copyright "TTM Setup Stats v3 Research"
#property link      ""
#property version   "1.00"
#property indicator_chart_window
#property indicator_plots 0

#define PREFIX "TTM_SETUP_STATS_V3_RESEARCH_"
#define DIR_BULL 1
#define DIR_BEAR -1
...
input int RectangleLengthBars = 70;
...
void DrawVerticalLine(const string name, const datetime lineTime, const double priceA, const double priceB, const color lineColor)
{
   if(!CreateObjectChecked(name, OBJ_TREND, lineTime, priceA, lineTime, priceB))
      return;
...
void CalculateRROutcome(const int bosShift, const int direction, const double fvgTop, const double fvgBottom, const datetime entryTime, const double entryPrice, const int rates_total, const datetime &time[], const double &high[], const double &low[], double &slPrice, double &tp1Price, double &tp2Price, double &tp3Price, datetime &rrEndTime, bool &hitSL, bool &hit1R, bool &hit2R, bool &hit3R)
{
   ...
   for(int shift = bosShift - 1; shift >= entryShift; shift--)
   {
      int index = IndexFromNewest(shift, rates_total, time);
      if(!BarTouchesFVG(high[index], low[index], fvgTop, fvgBottom))
         continue;
      ...
      if(direction == DIR_BULL)
         slPrice = MathMin(slPrice, low[index]);
      else
         slPrice = MathMax(slPrice, high[index]);
   }
   ...
   int rrEndShift = MathMax(0, entryShift - RRBarLengthBars);
   rrEndTime = time[IndexFromNewest(rrEndShift, rates_total, time)];

   int maxForwardShift = MathMax(0, entryShift - StatsForwardBars);

   for(int shift = entryShift - 1; shift >= maxForwardShift; shift--)
   {
      int index = IndexFromNewest(shift, rates_total, time);
...
void ScanSetups(const int rates_total, const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[])
{
   ...
   for(int shift = maxShift; shift >= SwingStrength + MinLiquidityCandles + 2; shift--)
   {
      ...
      {
         int bosShift = -1;
         for(int findShift = shift - 1; findShift >= 1; findShift--)
         {
            int findIndex = IndexFromNewest(findShift, rates_total, time);
            if(time[findIndex] == bosTime)
            {
               bosShift = findShift;
               break;
            }
...
{
   DrawLabel(PREFIX + "DIAG_TITLE", 12, 20, "TTM Setup Stats v3 Research", TextColor);
...
int OnInit()
{
   IndicatorSetString(INDICATOR_SHORTNAME, "TTM Setup Stats v3 Research");
