#property copyright "TTM Setup Sequence v1"
#property link      ""
#property version   "1.00"
#property indicator_chart_window
#property indicator_plots 0

#define PREFIX "TTM_SETUP_SEQ_V1_"
#define DIR_BULL 1
#define DIR_BEAR -1
...
bool FindLiquidityAndBoS(const int fvgShift, const int direction, const double fvgTop, const double fvgBottom, const int rates_total, const datetime &time[], const double &high[], const double &low[], const double &close[], datetime &liquidityTime, double &liquidityPrice, double &breakPrice, datetime &bosTime)
{
...
void ScanSetups(const int rates_total, const datetime &time[], const double &high[], const double &low[], const double &close[])
{
   ...
   for(int shift = maxShift; shift >= SwingStrength + MinLiquidityCandles + 2; shift--)
   {
      ...
      if(!FindLiquidityAndBoS(shift, direction, top, bottom, rates_total, time, high, low, close, liquidityTime, liquidityPrice, breakPrice, bosTime))
         continue;
...
void DrawSetup(const TTMSetup &setup)
{
   ...
   string directionText = setup.direction == DIR_BULL ? "Bullish" : "Bearish";
...
{
   DrawLabel(PREFIX + "DIAG_TITLE", 12, 20, "TTM Setup Sequence v1", TextColor);
   ...
   DrawLabel(PREFIX + "DIAG_SETUPS", 12, 92, "Displayed setups: " + IntegerToString(g_setupsDisplayed), TextColor);
   DrawLabel(PREFIX + "DIAG_OBJECTS", 12, 110, "Objects created: " + IntegerToString(g_objectsCreated), TextColor);
   DrawLabel(PREFIX + "DIAG_FAILURES", 12, 128, "Create failures: " + IntegerToString(g_objectCreateFailures), TextColor);
   DrawLabel(PREFIX + "DIAG_ERROR", 12, 146, "Last object error: " + IntegerToString(g_lastObjectError), TextColor);
}
...
int OnInit()
{
   IndicatorSetString(INDICATOR_SHORTNAME, "TTM Setup Sequence v1");
                ...
                const int &spread[])
{
   ...
   ScanSetups(rates_total, time, high, low, close);
   DrawAllSetups();
